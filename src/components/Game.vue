...
watch(groundX, (x, oldX) => {
  ...
  if (pipes.value.length && pipes.value[0].x < x - 100) pipes.value.shift();
});
...
watch(groundX, (x, prevX) => {
  ...
  if (
    top.contains(x + birdX.value, birdY.value) ||
    bottom.contains(x + birdX.value, birdY.value)
  ) {
    audios.hit.play();
...
watch(groundX, (x, prevX) => {
  ...
  birdX.value += prevX - x;
  birdRotation.value += 0.15;
  if (birdX.value < -100) emit("over");
});
</script>
